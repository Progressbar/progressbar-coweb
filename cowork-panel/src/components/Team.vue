  ...
  <div class="Team">
    <div class="columns is-centered">
      <div class="column is-4 content has-text-white">
        <img width="320px" src="/static/img/team/matej_nemcek.jpg">
        ...
        <span class="social-icons" v-for="item in socialsYangwao.slice(0,3)" :key="item.url">
          <a :href="item.url" target="_blank" rel="noopener" class="social-icon" aria-hidden="true">
            <font-awesome-icon v-if="item.icon" :icon="item.icon" />
          </a>
        </span><br>
        <span class="social-icons" v-for="item in socialsYangwao.slice(3,6)" :key="item.url">
          <a :href="item.url" target="_blank" rel="noopener" class="social-icon" aria-hidden="true">
            <font-awesome-icon v-if="item.icon" :icon="item.icon" />
          </a>
        </span>
      ...
      <div class="column is-4 content has-text-white">
        <img width="320px" src="/static/img/team/anett_rolikova.jpg">
        <h3 class="has-text-white">
          Anett Rolikova
        </h3>
        <p>
        ...
        <span class="social-icons" v-for="item in socialsAnett.slice(0,3)" :key="item.url">
          <a :href="item.url" target="_blank" rel="noopener" class="social-icon" aria-hidden="true">
            <font-awesome-icon v-if="item.icon" :icon="item.icon" />
          </a>
        </span><br>
        <span class="social-icons" v-for="item in socialsAnett.slice(3,6)" :key="item.url">
          <a :href="item.url" target="_blank" rel="noopener" class="social-icon" aria-hidden="true">
            <font-awesome-icon v-if="item.icon" :icon="item.icon" />
          </a>
        </span>
    ...
    data () {
      return {
        ...
        socialsYangwao: [
          ...
          { url: 'https://m.me/matej.wao.nemcek', title: 'Messenger', icon: faFacebookMessenger }
        ],
        socialsAnett: [
